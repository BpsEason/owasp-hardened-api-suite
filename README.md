...
def parse_junit_xml(file_path):
    ...
    try:
        ...
        for suite in test_suites:
            ...
            total_failed += int(suite.get('failures', 0)) + int(suite.get('errors', 0))
        ...
        logger.error(f"解析 JUnit XML 報告失敗 {file_path}: {e}")
        return None

def generate_markdown_report(pytest_data, phpunit_data, zap_data):
    """生成 Markdown 報告."""
    report_content = [f"## OWASP Hardened API Suite Security Report\n\n"]
    report_content.append(f"**生成日期：** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    report_content.append("### 🧪 FastAPI Pytest 安全測試結果\n\n")
    if pytest_report_data:
        report_content.append(f"- 總測試數量: **{pytest_data.total_tests}**\n")
        report_content.append(f"- 失敗測試數量: **{pytest_data.total_failed}**\n\n")
        for test in pytest_data['test_results']:
            status_emoji = "✅" if test['status'] == 'PASS' else "❌"
            report_content.append(f"| `{test['test_name']}` | {status_emoji} {test['status']} | {test['message']} |\n")
    else:
        report_content.append("未找到有效的 FastAPI Pytest 報告。\n\n")
    return ''.join(report_content)

if __name__ == '__main__':
    pytest_report_path = os.getenv('PYTEST_REPORT_PATH', 'pytest-report.xml')
    phpunit_report_path = os.getenv('PHPUNIT_REPORT', 'laravel-report.xml')
    zap_report_path = os.getenv('ZAP_REPORT_PATH', 'zap-reports/zap_report.json')
    output_report_path = os.getenv('OUTPUT_REPORT_PATH', 'reports/summary.md')
    pytest_data = parse_junit_report(pytest_report_path)
    phpunit_data = parse_junit_report(phpunit_report_path)
    ...
    logger.info(f"安全報告已生成至: {output_report_path}")
```
...
**Q2: 這個專案的目的與主要目標是什麼？它解決了什麼問題？**  
**A**: 這個專案的主要目標有幾個方面。首先，它提供安全的 Laravel API 範例，支援範例整合 Sanctum API。認證據，第二，它使用 FastAPI 模擬態 SQL 注入、 XSS 和認證漏洞和攻擊漏洞。同時，它支援 SWASP API 動態安全漏洞掃描（需自行配置）。最終，它可以生成簡單的 Markdown 安全報告（需整合報告生成程式）。最終，我們希望解決的是，在 API 程式中，如何系統化的測試驗證安全性，並將安全測試融入開發流程中。

### 系統架構與技術棧網

**Q3: 能否解釋一下這個專案的系統架構？各個組件負責什麼角色？**  
**A**: 這個專案預期的系統架構是一個容器化的微服務組合。最前端是前端，Nginx，負責處理用戶的 HTTP 請求並代理到 PHP-FPM + Laravel API。PHP-FPM + Laravel API 提供產品查詢，提供產品評論，防禦 XSS 和 SQL 注入，並與 MariaDB 互動儲存資料。FastAPI Tester 是一個獨立的服務，獨立服務，用於模擬攻擊，發送請求到 Nginx，驗證 API 安全性。OWASP ZAP 對 Nginx 進行動態掃描，生成 JSON 報告。Report Generator 整合 PHPUnit、Pytest 和 ZAP 報告，生成 Markdown 報告。所有服務運行在 Docker 網絡中，安全測試由開發者或 CI/CD 觸發。

**Q4: 這個專案使用了哪些核心技術？您為什麼選擇它們？**  
**A**: 專案的核心技術包括：後端 API 使用 PHP 8.2+ 和 Laravel 框架。安全測試部分採用 Python 3.9+ 和 pip 驅動的 FastAPI 測試器。資料庫選擇 MariaDB。整體環境通過 Docker 和 Docker Compose 進行容器化管理。這些技術的選擇是為了兼顧開發效率高、安全性和效率以及靈活性。例如，Laravel 提供了內建的安全功能，FastAPI 適合快速構建輕量級測試服務，而 Docker 確保了環境的一致性和可重複性。

### 安全防禦與測試

**Q5: 這個 Laravel API 專案是如何防禦 SQL 注入和 XSS 攻擊的？**  
**A**: 在 Laravel API 中，我們通過兩層防禦。對於 SQL 注入，使用 Laravel Eloquent 的查詢建構器，例如在 `ProductController` 的 `search` 方法中，`where('name', 'like', '%' . $request->input('name') . '%')` 自動使用參數綁定，避免 SQL 注入風險。對於 XSS，`ProductController` 使用 `Response::jsonEscaped()` 巨集，遍歷 JSON 數據，對字串進行 HTML 實體轉義，確保惡意腳本無法在前端執行。

**Q6: FastAPI Tester 在這個專案中扮演什麼角色？它是如何模擬攻擊的？**  
**A**: FastAPI Tester 是一個獨立的 Python 服務，扮演自動化安全測試客戶端角色，模擬惡意 HTTP 請求並分析 API 響應。例如，它使用 `AttackPayload` 模型構造攻擊數據，通過 `httpx` 發送請求。在 `simulate_xss` 中，發送包含 `<script>` 的內容，檢查是否被轉義為 `<script>`。測試與 Pytest 整合，自動運行並生成 `pytest-report.xml` 報告。

**Q7: 專案中是如何整合動態應用程式安全測試（DAST）的？**  
**A**: 我們使用 OWASP ZAP 作為 DAST 工具（需自行配置）。通過 `zap-cli` 命令，ZAP 指向 Laravel API 的 `openapi.json`，執行 Spider 和 Active Scan，檢測潛在漏洞，並生成 JSON 報告。報告作為 CI/CD 的 Artifact，供後續報告生成使用。

### CI/CD 與報告

**Q8: 專案中的 CI/CD 流程是怎樣的？各階段的作用是什麼？**  
**A**: 專案預期的 CI/CD 流程包含 `build`、`test`、`security_scan` 和 `report` 階段（需自行撰寫 `.gitlab-ci.yml`）。`build` 構建 Laravel 和 FastAPI 的 Docker 映像；`test` 運行 PHPUnit 和 Pytest 測試；`security_scan` 使用 ZAP 掃描 API；`report` 運行 `generate_report.py`，整合測試和掃描結果，生成 Markdown 報告。流程確保代碼提交自動驗證安全性。

**Q9: 專案如何生成最終的安全報告？報告中包含哪些資訊？**  
...
**A**: 挑戰包括：
- **Docker 環境複雜性**：初期配置 `docker-compose.yml` 時，服務通信和卷掛載權限有問題。解決方案是檢查服務名稱、網路配置和權限設定。
- **Laravel API 與 FastAPI Tester 的協同**：確保 FastAPI Tester 正確發送請求並解析響應，需精準測試邏輯。使用 `httpx` 和斷言比對字串解決。
...
- **增強報告**：生成 HTML 報告或整合至 SonarQube。
- **引入 SAST**：使用靜態程式碼分析工具檢查漏洞。
- **前端應用**：加入前端展示攻防效果。
- **雲部署**：支援多雲環境和運行時安全策略。
這些措施會使專案更貼近實際應用場景。
...
4. **如何整合 OWASP？**  
   設置 ZAP 容器，提供 API 的 `openapi.json`，參考「安全測試」中的 ZAP 命令。

## 注意事項
- 本專案僅提供核心代碼，需熟悉 Laravel 和 FastAPI 的開發者整合。
- 建議在 Linux 環境測試以確保 Docker 網路穩定。
- 若需 CI/CD 流程，需撰寫 `.gitlab-ci.yml`。

## 聯繫與貢獻
這是一個簡單的範例專案，僅提供核心功能。若有問題或建議，歡迎在 GitHub 提交 issue 或 PR。感謝您的支持！
